...
A three-sample clean-HEAD local `throughput` run at `4728a96` on Bun 1.2.22 and an Apple M4 Pro executed 6,144 mutations, verified 49,152 logical row deliveries, and completed 768 selective writes. Per-sample mutation throughput ranged from 274.86 to 279.78/s, logical delivery throughput from 52,584.10 to 55,717.89/s, selective writes from 485.33 to 494.47/s, and materializations from 399.64 to 414.10/s. This Miniflare run is regression telemetry, not a benchmark baseline, capacity claim, or performance target. The repository still requires repeated runs on a declared platform before setting targets.
