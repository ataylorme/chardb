...
This is engineering work intended for public review, not a database you should deploy yet. A declared organization mutation now crosses the public WebSocket, Gateway, Catalog, and Cdb path in a focused workerd test. An explicit organization query can authorize and read one exact partition from one Cdb and return one initial snapshot. That snapshot-only path does not register the query, replay missed changes, or send live updates, and no packed example has passed a full sign-in-to-read round trip.
...
- Public initial snapshots for explicit, exact-single-partition organization queries
...
- Persisting public query registrations and re-running them after committed writes
...
Application handlers can use only typed builders against registered `cdbTable` definitions. The wrapper rejects raw SQL, Drizzle session and client access, relational and count shortcuts, plain-table CRUD, insert-select, conflict methods, `returning`, and unsupported properties before or after policy attachment. Gateway can route an explicit exact-partition organization query through this read-only policy wrapper to one Cdb read and return one snapshot, including an empty snapshot. That path does not call the separate registration helpers or create live invalidation state.
...
The organization foreign key identifies the intended transaction and placement boundary. Related rows colocate through their foreign-key chain. Declared organization mutations enforce that boundary by checking the extracted organization against Catalog membership. An explicit organization query can cross the same authority boundary only when its declared partition and server-owned intent identify that exact organization and one virtual shard; it produces one snapshot, not a registered live query.
