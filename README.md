...
A three-sample clean-HEAD local `throughput` run at `34210bb` on Bun 1.2.22 and an Apple M4 Pro executed 6,144 mutations, replaced 24 live clients midway, verified 86,016 logical row deliveries, reconstructed 96 live Cdb registrations across three evictions, replaced them with 96 exact new SDK/Gateway registrations, and completed 768 selective writes after recovery. Per-sample mutation throughput ranged from 134.53 to 152.10/s, client churn took 210.33 to 227.76 ms for eight clients, logical delivery throughput ranged from 24,729.13 to 26,926.44/s, selective recovery took 255.14 to 275.14 ms for 32 registrations, writes ranged from 219.47 to 267.92/s, and materializations from 229.90 to 252.41/s. This Miniflare run is regression telemetry, not a benchmark baseline, capacity claim, or performance target. The repository still requires repeated runs on a declared platform before setting targets.
