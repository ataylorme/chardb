...
function useAuthSession() {
    const [snapshot, setSnapshot] = useState(() => authClient.useSession.get());
    useEffect(() => authClient.useSession.subscribe(() => setSnapshot(authClient.useSession.get())), []);
    return snapshot;
}
...
export function App() {
    ...
    return (
        <ChardbProvider endpoint={chardbEndpoint()} auth={authClient} logicalDb="chat" crossTab>
            <Workspace />
        </ChardbProvider>
    );
}
