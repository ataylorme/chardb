...
describe("generated tutorial flow", () => {
    ...
    test("matches the visible organization tutorial contract", async () => {
        ...
        for (const source of [generatedApp, chatApp]) {
            ...
            expect(source).toContain("useQuery(listMessages");
            expect(source).toContain("useMutation<");
            expect(source).toContain("<ChardbProvider");
        }
        ...
        expect(generatedApp).toContain("const session = authClient.useSession()");
        expect(generatedApp).toContain("const organizations = authClient.useListOrganizations()");
        ...
        expect(generatedApp).not.toContain('useSession } from "@chardb/core/react"');
        expect(generatedApp).toContain("userId={session.data.user.id}");
        expect(generatedApp).toContain("authClient.organization.create({");
        expect(generatedApp).toContain("authClient.organization.setActive({ organizationId");
        expect(generatedApp).toContain("authClient.organization.delete({ organizationId: activeOrganizationId })");
        expect(generatedApp).toContain('data-testid="delete-organization"');
        expect(generatedApp).toContain("userId={session.data.user.id}");
        expect(generatedApp).toContain("useQuery(listMessages, { organizationId, limit: 50 })");
        expect(generatedApp).toContain('fileRef("messages", "attachment")');
        expect(generatedApp).toContain("useFile(messageAttachment)");
