...
try {
    ...
    function spawnGeneratedDev(workerPort, webPort, output = "inherit") {
        return Bun.spawn([process.execPath, "run", "dev"], {
            ...
            env: {
                ...process.env,
                CHARDB_URL: `http://127.0.0.1:${workerPort}`,
                CHARDB_WEB_URL: `http://127.0.0.1:${webPort}`,
                CHARDB_PERSIST_TO: persistTo,
                CHARDB_ADMIN_TOKEN: "windows-dev-tree-admin",
                BETTER_AUTH_SECRET: "windows-dev-tree-auth-secret-at-least-32-characters",
