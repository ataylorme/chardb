...
On a host that permits local listeners, the current real-workerd compatibility run passes `gateway-live` 6/6 including the two default-small scale scenarios, `gateway-jwt` 21/21, and `gateway-snapshot` 1/1. The snapshot test is allowed 15 seconds because the same case starts Miniflare/workerd once, disposes it, and starts it again against the persisted directory. The timeout is test infrastructure budget, not a runtime latency guarantee. A restricted sandbox can fail while Miniflare binds ephemeral port 0 before application code runs; that environmental result is not a product compatibility claim.
...
The same configured harness has two default-small SDK scale scenarios. One fans deterministic mutations across clients in two organizations and proves exact tenant isolation through durable convergence. The other installs filtered subscriptions and refreshes each filter through repeated mutations. Both drive convergence from stored Gateway versions and outbox state, clean up registrations, and emit JSON timing telemetry. Environment variables can raise the bounded workload for manual runs. Timing never decides correctness, and the repository sets no performance target.
