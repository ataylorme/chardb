...
| --- | --- |
| [`Gateway`](src/server/do/gateway.ts) | Owns hibernated WebSockets, verifies JWT identity, authorizes declared organization mutations and exact-partition initial queries through Catalog, resolves refs in the server manifest, and batches outgoing results and patches. Persistent query subscriptions and presence remain closed. |
...
API helpers attach a `__chardbRef` and a kind marker to mutations, queries, crons, presence keys, and ledgers. Public organization mutations require an explicit literal `ref` alongside `authority: "organization"`. Public organization queries additionally require a partition key and server-owned intent. The Vite plugin preserves explicit refs and authority metadata in browser handles, rejects invalid or duplicate declarations, and uses generated module refs for APIs that do not require an explicit one. [`manifestFromExports`](src/server/manifest.ts) walks the marked server exports and builds maps for mutations, queries, crons, and ledgers.

Mutation and query descriptors retain raw argument validation, a validated handler entry point, the partition-key extractor, and the authority declaration. Query descriptors also retain intent extraction. `chardb()` closes over the same lazily built manifest in configured Gateway and Cdb classes. Gateway validates each public mutation or initial-query argument value once before deriving authority and routing metadata. Cdb resolves the validated entry point inside the shard isolate rather than applying the transform again. The manifest and its functions never cross RPC.
...
Gateway rechecks expiry and not-before before protected operations. Each socket receives a server-generated connection id. Auth refresh barriers use that id, serialize multiple refreshes, drain admitted mutations and initial queries before replacing identity, report delivered snapshot ids through `mustRefetch`, and gate later work. They do not re-run those queries. A failed refresh writes a terminal rejected attachment before closing the socket, so queued work cannot reuse stale verified state.

A Miniflare workerd test drives the configured Gateway Durable Object and WebSocket with ES256 tokens, a real Catalog SQLite cache, Catalog-derived organization authority, and a configured Cdb. The test seeds JWKs and auth rows through test-only HTTP routes. Better Auth sign-in, outbound JWKS fetch, cache refresh, and key rotation remain untested. An explicit organization query can use that authority only when its validated partition and server intent resolve to the same exact organization and one virtual shard. Undeclared, mismatched, scatter, and cross-partition queries, persistent subscriptions, and presence remain closed. Anonymous query behavior is undefined.

## Subscription design

Wire protocol v3 requires `protocolV` in `hello` and `welcome`. Its decoder rejects unknown fields, missing fields, and incorrect field types. A subscription request sends a server-stamped query ref and raw arguments. It does not send intent or a query hash. The protocol also defines a `snapshot` with a subscription id, cookie, and JSON row array. The client replaces its rows, clears optimistic patches, records the cookie, and marks the subscription `live` when that message arrives. The server now emits that snapshot, including an empty row array, for the narrow initial organization query path.

Separate isolated registration helpers can resolve the ref in a local manifest, validate arguments, derive intent, compute the current query hash, persist a registration, and choose shards:
...
Each selected Cdb can persist the subscription's composite Gateway, client, and subscription id along with its principal, ref, arguments, tables, and intervals. The in-memory [`IntervalMap`](src/intervals.ts) uses that composite identity and rebuilds from SQLite when Cdb starts. Focused reconstruction tests cover colliding numeric subscription ids and targeted unsubscribe. `matchSubsForRow` maps a committed row's indexed keys back to composite registrations. Gateway code exists to coalesce patches before sending a `poke`. The public initial-snapshot path does not call these registration helpers or persist delivery state.

The current registration hash serializes the ref, validated arguments, and derived intent. It still needs canonical JSON and verified principal, tenant, auth epoch, and policy epoch inputs. Public registration remains disconnected even though one-shot organization reads now have Catalog authority.

Gateway can authorize one explicit organization query, require its validated partition and server-owned intent to identify the same exact organization and one virtual shard, route that shard through Catalog, call `Cdb.query` once, and emit one protocol-v3 snapshot. `Cdb.query` constructs a read-only policy-wrapped Drizzle database, invokes the validated handler with Catalog-derived auth, and requires a JSON array result. Focused workerd coverage includes nonempty and empty snapshots, other-tenant denial, malformed routes, cancellation, and auth refresh ordering. The path creates no server registration. Committed mutations do not invalidate or re-run it, and resume cannot replay missed changes. React presence, upload, stream, and vector hooks are not exported until their client paths exist.
