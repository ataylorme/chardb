...
`useQuery` identifies visible data by the client object, query ref, and canonical argument identity. A change to any of them makes the render pending even before effect cleanup runs. The old subscription is unsubscribed, and an active-listener fence prevents a late callback from republishing rows under the new identity.
