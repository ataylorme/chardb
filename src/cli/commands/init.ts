...
The scaffold uses Wrangler's native same-Worker loopback exports, so its internal Durable Object classes need migrations but no application-visible Durable Object bindings.

The current chardb package applies forward maintenance-mode domain migrations and runs one authenticated organization mutation and live-query path. It is still experimental. Do not deploy this scaffold with production data.
`;
...
// and the Durable Object classes Wrangler exposes through ctx.exports.
...
export async function runInit(ctx: CliContext, opts: InitOptions): Promise<void> {
    ...
    await ctx.write(`${ctx.cwd}/wrangler.toml`, `${wrangler}\n`);
