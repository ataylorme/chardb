...
Start the complete local app in three commands:
...
\`bun run dev\` starts Wrangler, reads the packaged schema version from \`/health\`, applies that migration target, and starts Vite. It prints the browser URL only after both processes are ready. Press Ctrl+C to stop both.
...
\`bun run test\` runs the generated Worker inside Cloudflare's Vitest integration. The test reads \`wrangler.toml\`, migrates a fresh Miniflare database, checks the native Durable Object and R2 bindings, signs in through Better Auth, creates an organization, and writes and reads one organization message. Cloudflare's in-process \`exports.default.fetch()\` cannot route a Durable Object's public-origin JWKS fetch back to the same Worker. The test captures the real Better Auth JWKS response first, then uses Cloudflare's MSW integration for that one outbound request. No signing key or JWT verification is faked.

The scaffold declares Chardb's four same-Worker Durable Object bindings in \`wrangler.toml\`. Chardb uses those namespaces internally; application routes use only the exported \`DB\` binding.

\`src/migrations/v1.json\` and \`src/migrations/v1.ts\` are the generated immutable version-one snapshot. Do not edit generated migration files after deployment. After each additive schema change, run \`bunx @chardb/core migrations generate --name <name>\` to append the next sequential version. The generator verifies the full JSON digest chain, every generated TypeScript file, and the exact journal before it writes. It accepts new tables, nonunique indexes, and nullable unconstrained columns. It rejects gaps, edits to old history, drops, renames, type or constraint changes, new unique indexes, and changes to existing file or vector resources. Existing application traffic stays closed until an explicit migration finishes.

The generated browser signs in anonymously, creates and switches organizations through Better Auth, uploads image attachments to Wrangler's native local R2 bucket, posts an organization message, receives its live replacement, and reloads persisted state through the packaged React hooks.

Remote deployment is explicit and resumable:
...
export const replaceMessageAttachment = api.mutation({
  ref: "src/api.ts#replaceMessageAttachment",
  authority: "organization",
  args: z.object({
    id: z.string(),
    organizationId: z.string(),
