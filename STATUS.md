...
| Shard-local query execution | Isolated | Direct `Cdb.query` resolves a registered query, supplies a read-only Drizzle database and the request auth context, executes the handler, and validates the JSON result. Focused Bun tests cover persisted rows, empty results, failures, and blocked write, raw SQL, and transaction entry points. Gateway does not route public subscriptions to this RPC or establish its auth authority. |
...
| Row and column authorization | Partial | Inserts enforce create grants, tenant and self authority, and writable columns. Updates enforce update grants and columns and make managed authority columns immutable. Updates and deletes AND tenant and self predicates with the caller's `where`, including operations with no `where`, and both require a matching grant. Workerd transaction tests cover rollback on forbidden update and delete operations. Select, raw SQL, and public routing remain outside this enforcement path. |
