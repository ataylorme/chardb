...
import { useFile } from "@chardb/core/react";

const messageAttachment = fileRef("messages", "attachment");

function Attachment({ organizationId, rowId }: Props) {
  const attachment = useFile(messageAttachment);

  return (
    <a href={attachment.downloadUrl({ organizationId, rowId })}>
      Download attachment
    </a>
  );
}
...
Upload and download use same-origin Worker routes. The browser sends the Better Auth session cookie automatically. Chardb checks the active organization, current membership, table policy, row ID, and opaque file ID before returning bytes.
