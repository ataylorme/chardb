...
Restore validates the manifest and topology, fences new traffic, arms every shard, and restarts the Durable Objects at their recorded bookmarks. Test the procedure against a disposable deployed Worker. Native PITR is not available in Miniflare.

The recovery point covers Durable Object SQLite. It does not rewind R2 or Vectorize. Copy objects to an independent retention bucket before deletion when file restoration matters; Chardb does not do that automatically. Treat Vectorize as a derived index and plan to rebuild it from authoritative SQLite vector heads after a restore.
